"""
Generate MP4 videos of Nelson-Siegel-Svensson term structure evolution.
"""
import sys
import math
import webbrowser
...
def make_nss_video(
    ...
    """
    Generate an MP4 video of NSS term structure evolution.
    ...
    exclude_short_for_fit : int
        Exclude contracts with maturity < this value from NSS estimation
    grid_step : int
        Step size for NSS evaluation grid
    frame_stride : int
    ...
    """
    print(f"\n{'='*60}")
    print(f"Generating NSS video for {commo_ticker}")
    ...
    y_range = (y_lo - y_pad, y_hi + y_pad + 10)  # Add extra +10 to upper bound
    
    print(f"Y-axis range: [{y_range[0]:.2f}, {y_range[1]:.2f}]")
    print(f"X-axis range: [{min_maturity}, {max_maturity}]")
    
    # NSS evaluation grid starting from min_maturity
    nss_grid = np.arange(min_maturity, max_maturity + grid_step, grid_step)
    ...
    line_nss, = ax.plot([], [], color='#FFA500', linewidth=1.5, label='NSS Fit')  # Orange, thinner
    ...
    # Pre-compute NSS params for all frames (with progress bar)
    print("\nPre-fitting NSS curves...")
    params_cache = {}
    
    for date in tqdm(dates, desc="Fitting NSS"):
        try:
            ...
            # Fit NSS only on contracts >= exclude_short_for_fit
            ...
            params = NSS_estimation(m_fit, p_fit, verbosity=False)
    ...
    def update_frame(frame_idx: int):
        ...
        try:
            ...
            if params is not None:
                # Evaluate NSS on grid starting from min_maturity
                nss_log = NSS_rate(nss_grid, params)
                nss_prices = np.exp(nss_log)
                line_nss.set_data(nss_grid, nss_prices)
        ...
        return line_nss, scatter_actual, date_text, params_text, timeline_cursor
    ...
    if output_path is None:
        output_filename = f"nss_video_{commo_ticker}_{start_date}_{end_date}_stride{frame_stride}.mp4"
    ...
    writer = writer_class(fps=fps, metadata={'artist': 'NSS Video Generator'})
